...
function event:ToString(tbl)
	...
	local str
	if weapon then
		str = string.format("%s [%s] has damaged %s [%s] for %s HP with %s.", tbl[3], team.GetName(tbl[4]), tbl[1], team.GetName(tbl[2]), tbl[5], weapon) 
	else
		str = string.format("%s [%s] has damaged %s [%s] for %s HP with an unknown weapon.", tbl[3], team.GetName(tbl[4]), tbl[1], team.GetName(tbl[2]), tbl[5]) 
	end
	return str
